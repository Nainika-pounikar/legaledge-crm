...
function ChartCard({ title, canvasRef }) {
  return (
    <div className="card" style={{ padding: '24px' }}>
      ...
      <div className="chart-wrap" style={{ height: '300px', position: 'relative' }}>
...
export default function SalesAnalytics() {
  ...
  const handleExport = () => {
    const rows = [
      ['Metric', 'Value'],
      ['Total Revenue', fmtINR(totalRevenue)],
      ['Pipeline Value', fmtINR(totalPipeline)],
      ['Average Deal Size', fmtINR(Math.round(avgDeal))],
      ['Win Rate', `${winRate}%`],
    ];
    const csv = rows.map(cols => cols.map(value => `"${String(value).replaceAll('"', '""')}"`).join(',')).join('\n');
    ...
    <div className="page page-fade">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Performance insights and sales intelligence</p>
        </div>
        <div className="page-actions">
          <button className="btn-secondary" onClick={handleExport}><i className="fa fa-download"></i> Export Data</button>
        </div>
      </div>
      ...
      <div className="dashboard-charts" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '24px' }}>
        <ChartCard title="Revenue Trend" canvasRef={lineRef} />
        <ChartCard title="Team Performance" canvasRef={barRef} />
      </div>
      <div className="dashboard-charts" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '24px', marginTop: '24px' }}>
