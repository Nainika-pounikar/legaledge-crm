...
const INITIAL_VIDEOS = [
  { id: 1, title: 'CRM Platform Overview', duration: '2:45', views: 1250, plays: 890, folder: 'Product Demos', date: '2026-03-10' },
    ...
    <div className="page-fade">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Host, manage, and track your podcast content</p>
        </div>
        <button className="btn-primary" onClick={() => { setForm({ title: '', folder: 'Tutorials', duration: '' }); setErr({}); setModalOpen(true); }}>
          <i className="fa fa-upload" /> Upload Podcast
        </button>
      </div>
      ...
      <div className="cards-grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
        {rows.map(v => (
          <div key={v.id} className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ height: '160px', background: '#2c3e50', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ padding: '15px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border-color)', paddingTop: '15px' }}>
                <button className="btn-secondary btn-sm" onClick={() => { navigator.clipboard.writeText(`<iframe src="/video/${v.id}"></iframe>`); alert('Embed code copied!'); }}>Copy Embed Code</button>
